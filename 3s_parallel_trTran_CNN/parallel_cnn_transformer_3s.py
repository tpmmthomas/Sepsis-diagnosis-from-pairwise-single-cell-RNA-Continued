...
xtrain = r"/uac/y19/khto9/ai/CNN/training_sample_3s.csv.gz"
ytrain = r"/uac/y19/khto9/ai/CNN/training_label_3s.csv.gz"
xtest = r"/uac/y19/khto9/ai/CNN/testing_sample2_3s.csv.gz"
ytest = r"/uac/y19/khto9/ai/CNN/testing_label_3s.csv.gz"
...
#hyper parameter
	...
	inputs0 = layers.Input(shape=(maxlen,))
	re=layers.Reshape((maxlen, 1))(inputs0)
	conv1 = layers.Conv1D(filters=32, kernel_size=4, activation='relu')(re)
	x = layers.BatchNormalization()(conv1)
	pool1 = MaxPooling1D(pool_size=3)(x)
	conv1 = Conv1D(filters=32, kernel_size=4, activation='relu')(pool1)
	x = layers.BatchNormalization()(conv1)
	pool1 = MaxPooling1D(pool_size=3)(x)
	conv1 = Conv1D(filters=32, kernel_size=4, activation='relu')(pool1)
	x = layers.BatchNormalization()(conv1)
	...
	merged = layers.concatenate([flat1,x])

	x = layers.Dropout(0.1)(merged)
	x = layers.Dense(200, activation="relu")(x)
	outputs = layers.Dense(1, activation="sigmoid")(x)

	
	
	model = keras.Model(inputs=[inputs0,inputs], outputs=outputs)
...
testresult=model.predict([x_test,x_test])
...
plt.title('AUROC curve for CNN + traditional Transformer')
plt.legend(loc="lower right")
plt.savefig("/uac/y19/khto9/ai/report/3sAUROCParallelCnnTrTransformer.png")
...
plt.title('AUPRC curve for CNN + traditional Transformer')
plt.legend(loc="lower right")
plt.savefig("/uac/y19/khto9/ai/report/3sAUPRCParallelCnnTrTransformer.png")
...
#model.save('/uac/y19/khto9/ai/report/saved_model/parallel_cnn_tran_3s_new.hp5',save_format="h5")
